...
public class ActPubFactory extends ServiceBase {
	 ...
	 */
	public APObj newCreateForNote(String userDoingAction, HashSet<String> toUserNames, String fromActor, String inReplyTo, String content, String noteUrl, String repliesUrl, boolean privateMessage, APList attachments) {
		ZonedDateTime now = ZonedDateTime.now(ZoneOffset.UTC);
		// log.debug("sending note from actor[" + fromActor + "] inReplyTo[" + inReplyTo);
		APObj payload = newNote(userDoingAction, toUserNames, fromActor, inReplyTo, content, noteUrl, repliesUrl, now, privateMessage, attachments);
	 ...
	 */
	public APObj newNote(String userDoingAction, HashSet<String> toUserNames, String attributedTo,  /* fromActor */String inReplyTo, String content, String noteUrl, String repliesUrl, ZonedDateTime now, boolean privateMessage, APList attachments) {
		if (content != null) {
			// convert all double and single spaced lines to <br> for formatting, for servers that don't
			// understand Markdown
			content = content.replace("\n", "<br>");
		}
		APObj ret = new APONote(noteUrl, now.format(DateTimeFormatter.ISO_INSTANT), attributedTo, null, noteUrl, repliesUrl, false, content, null);
		if (inReplyTo != null) {
	 ...
	 */
	public APOCreate newCreate(String userDoingAction, APObj object, String fromActor, HashSet<String> toUserNames, String noteUrl, ZonedDateTime now, boolean privateMessage) {
	...
	public void setRecipients(String fromActor, String userDoingAction, APObj object, HashSet<String> toUserNames, boolean privateMessage, boolean includeTags) {
	...
	public APOUpdate newUpdate(String userDoingAction, APObj object, String fromActor, HashSet<String> toUserNames, String objUrl, ZonedDateTime now, boolean privateMessage) {
		String idTime = String.valueOf(now.toInstant().toEpochMilli());
	...
	public APObj makeAPONote(MongoSession as, SubNode child, SubNode parent) {
		...
		APONote ret = new APONote(nodeIdBase + hexId, published, actor, null, nodeIdBase + hexId, repliesUrl, false, content, new APList().val(APConst.CONTEXT_STREAMS_PUBLIC));
	...
	public APObj makeAPOCreateNote(MongoSession as, String userName, String nodeIdBase, SubNode child) {
		...
		String repliesUrl = prop.getProtocolHostAndPort() + APConst.PATH_REPLIES + "/" + hexId;
		APObj ret = new APONote(nodeIdBase + hexId, published, actor, null, nodeIdBase + hexId, repliesUrl, false, content, new APList().val(APConst.CONTEXT_STREAMS_PUBLIC));
		if (parent != null) {
	...
	public APOPerson generatePersonObj(SubNode userNode) {
		...
		try {
			...
			String avatarUrl = prop.getProtocolHostAndPort() + AppController.API_PATH + "/bin/avatar" + "?nodeId=" + userNode.getIdStr() + "&v=" + avatarVer;
			APOPerson actor =  //
			/*
			...
			//
			//
			new APOPerson().put(APObj.id, apUtil.makeActorUrlForUserName(userName)).put(APObj.did, did).put(APObj.preferredUsername, userName).put(APObj.name, displayName).put(APObj.published, DateUtil.isoStringFromDate(userNode.getCreateTime())).put(APObj.icon,  //
			//
			//
			...
			if (headerAtt != null) {
				...
				if (headerImageMime != null) {
					...
					if (headerImageVer != null) {
						String headerImageUrl = prop.getProtocolHostAndPort() + AppController.API_PATH + "/bin/profileHeader" + "?nodeId=" + userNode.getIdStr() + "&v=" + headerImageVer;
						actor.put(APObj.image,  //
						//
						//
						new APObj().put(APObj.type, APType.Image).put(APObj.mediaType, headerImageMime).put(APObj.url, headerImageUrl));
					}
				}
			}
			//
			...
			//
			//
			actor.put(APObj.summary, userNode.getStr(NodeProp.USER_BIO)).put(APObj.inbox, host + APConst.PATH_INBOX + "/" + userName).put(APObj.outbox, host + APConst.PATH_OUTBOX + "/" + userName).put(APObj.followers, host + APConst.PATH_FOLLOWERS + "/" + userName).put(APObj.following, host + APConst.PATH_FOLLOWING + "/" + userName).put(APObj.url, host + "/u/" + userName + "/home").put(APObj.endpoints, new APObj().put(APObj.sharedInbox, host + APConst.PATH_INBOX)).put(APObj.publicKey, new APObj().put(APObj.id, apStr(actor, APObj.id) + "#main-key").put(APObj.owner, apStr(actor, APObj.id)).put(APObj.publicKeyPem, "-----BEGIN PUBLIC KEY-----\n" + publicKey + "\n-----END PUBLIC KEY-----\n")).put(APObj.supportsFriendRequests, true);
