...
public class ActPubUtil extends ServiceBase {
    ...
    public APObj getJson(String url, Class<?> clazz, MediaType mediaType, HttpHeaders headers) {
        ...
        HttpClientErrorException.Gone goneEx) {
            ...
            log.debug("failed getting json: " + url + " -> " + e.getMessage() + " ex.class=" + e.getClass().getName() + " respCode=" + responseCode);
            return null;
        }
        return ret;
    }
    ...
    public void iterateCollection(MongoSession ms, String userDoingAction, Object collectionObj, int maxCount, ActPubObserver observer) {
    ...
    public GetThreadViewResponse getNodeThreadView(MongoSession ms, String nodeId, boolean loadOthers) {
        ...
        while (node != null && (nodes.size() < MAX_THREAD_NODES)) {
            try {
                 ...
                 */
                boolean topNode = node.isType(NodeType.POSTS) || node.isType(NodeType.ACT_PUB_POSTS) || node.isType(NodeType.ACCOUNT);
                if (!topNode) {
                    ...
                    if (loadOthers) {
                        Iterable<SubNode> iter = read.getChildren(ms, node, Sort.by(Sort.Direction.DESC, SubNode.CREATE_TIME), 20, 0);
                        ...
                        for (SubNode child : iter) {
                            if (!child.getId().equals(lastNodeId)) {
                                ...
                                child, false, Convert.LOGICAL_ORDINAL_IGNORE, false, false, false, false, true, true, null, false));
                            }
                        }
                        /*
                        ...
                        NodePath.USERS_PATH + "/(" + NodePath.LOCAL + "|" + NodePath.REMOTE + ")", NodeProp.INREPLYTO.s(), replyTargetId);
                        for (SubNode child : iter) {
                            // if we didn't already add above, add now
                            if (!childIds.contains(child.getIdStr())) {
                                children.add(convert.convertToNodeInfo(false, ThreadLocals.getSC(), ms, child, false, Convert.LOGICAL_ORDINAL_IGNORE, false, false, false, false, true, true, null, false));
                            }
                        }
                        if (children.size() > 0) {
            ...
            if (ownerActorUrl != null) {
                return (SubNode) arun.run(as -> {
                    ...
                    if (accountNode != null) {
                        ...
                        SubNode outboxNode = read.getUserNodeByType(as, apUserName, accountNode, "### Posts", NodeType.ACT_PUB_POSTS.s(), Arrays.asList(PrivilegeType.READ.s(), PrivilegeType.WRITE.s()), NodeName.POSTS);
                        if (outboxNode == null) {
                            log.debug("no outbox for user: " + apUserName);
                            return null;
                        }
                        node = apub.saveInboundForeignObj(as, userDoingAction, accountNode, outboxNode, obj, APType.Create, null, null, false, null);
                    }
                    return node;
                });
            }
