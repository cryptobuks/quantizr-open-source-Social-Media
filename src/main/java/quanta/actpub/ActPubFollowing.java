...
public class ActPubFollowing extends ServiceBase {
    ...
    public void setFollowing(String followerUserName, String apUserName, boolean following) {
        try {
            apLog.trace("Local Follower User (person doing the following): " + followerUserName + " setFollowing: " + apUserName + "following=" + following);
            ...
            arun.run(as -> {
                ...
                if (actorUrlOfUserBeingFollowed == null) {
                    actorUrlOfUserBeingFollowed = apub.getUserProperty(as, followerUserName, apUserName, null, NodeProp.ACT_PUB_ACTOR_URL.s());
                ...
                APOFollow followAction = new APOFollow(prop.getProtocolHostAndPort() + "/follow/" + String.valueOf(new Date().getTime()), sessionActorUrl, actorUrlOfUserBeingFollowed);
                APObj action = null;
                // send follow action
                if (following) {
                    action = followAction;
        ...
        Runnable runnable = () -> {
            arun.<APObj>run(as -> {
                try {
                    ...
                    SubNode followerFriendList = read.getUserNodeByType(as, followerUserName, null, null, NodeType.FRIEND_LIST.s(), null, NodeName.FRIENDS);
                    /*
                    ...
                    exec.run(() -> {
                        ...
                        apUtil.securePostEx(apStr(followerActor, APObj.inbox), privateKey, _actorBeingFollowedUrl, accept, APConst.MTYPE_LD_JSON_PROF);
    ...
    public APOOrderedCollectionPage generateFollowingPage(String userName, String minId) {
        ...
        APOOrderedCollectionPage ret = new APOOrderedCollectionPage(url, following, prop.getProtocolHostAndPort() + APConst.PATH_FOLLOWING + "/" + userName, following.size());
        return ret;
    }
     ...
     */
    public List<String> getFollowing(String userName, boolean foreignUsers, boolean localUsers, String minId, boolean queueForRefresh, HashSet<ObjectId> blockedUserIds) {
    ...
    public GetFollowingResponse getFollowing(MongoSession ms, GetFollowingRequest req) {
        ...
        return arun.run(as -> {
            ...
            for (SubNode node : iterable) {
                NodeInfo info = convert.convertToNodeInfo(false, ThreadLocals.getSC(), as, node, false, counter + 1, false, false, false, false, false, false, null, false);
                if (info != null) {
    ...
    private Query findFollowingOfUser_query(MongoSession ms, String userName) {
        ...
        SubNode friendsListNode = read.getUserNodeByType(ms, userName, null, null, NodeType.FRIEND_LIST.s(), null, NodeName.FRIENDS);
        ...
        Criteria crit = //
        //
        Criteria.where(SubNode.PATH).regex(mongoUtil.regexDirectChildrenOfPath(friendsListNode.getPath())).and(SubNode.TYPE).is(NodeType.FRIEND.s());
        q.addCriteria(crit);
        return q;
    }
}
