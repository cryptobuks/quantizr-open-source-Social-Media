...
public class MongoDelete extends ServiceBase {
	...
	public long deleteOldActPubPosts(int monthsOld, MongoSession ms) {
		...
		Criteria crit =  //
		Criteria.where(SubNode.PROPS + "." + NodeProp.ACT_PUB_OBJ_TYPE).ne(null).and(SubNode.MODIFY_TIME).lt(date);
		q.addCriteria(crit);
		DeleteResult res = ops.remove(q, SubNode.class);
	...
	public long deleteOldNostrPosts(int monthsOld, MongoSession ms) {
		...
		Date date = Date.from(ldt.atStartOfDay(ZoneId.systemDefault()).toInstant());
		Criteria crit =  //
		Criteria.where(SubNode.PROPS + "." + NodeProp.OBJECT_ID).regex("^\\.").and(SubNode.MODIFY_TIME).lt(date);
		q.addCriteria(crit);
		DeleteResult res = ops.remove(q, SubNode.class);
		return res.getDeletedCount();
	}
	...
	public void cleanupOldTempNodesForUser(MongoSession ms, SubNode userNode) {
		...
		Date date = Date.from(ldt.atStartOfDay(ZoneId.systemDefault()).toInstant());
		Criteria crit =  //
		Criteria.where(SubNode.PATH).regex(mongoUtil.regexRecursiveChildrenOfPath(userNode.getPath())).and(SubNode.MODIFY_TIME).lt(date); //
		q.addCriteria(crit);
	...
	public DeleteNodesResponse bulkDeleteNodes(MongoSession ms) {
		...
		// root.
		Criteria crit = Criteria.where(SubNode.OWNER).is(userNode.getOwner()).and(SubNode.PATH).not().regex(mongoUtil.regexRecursiveChildrenOfPathIncludeRoot(userNode.getPath()));
		q.addCriteria(crit);
	...
	public void deleteMatches(MongoSession ms, SubNode node, String prop, String text, boolean fuzzy, boolean caseSensitive, String timeRangeType, boolean recursive, boolean requirePriority) {
